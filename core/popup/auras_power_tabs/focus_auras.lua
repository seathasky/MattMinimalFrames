function MMF_BuildAurasPowerFocusAurasSection(ctx)
    ...
    aurasTitle:SetText("FOCUS AURA POSITION")
    ...
    local buffsCheckbox = CreateMinimalCheckbox(root, "Buffs", AURA_COL_X, BUFF_TOGGLE_Y, "showFocusBuffs", true, function()
    ...
    local debuffsCheckbox = CreateMinimalCheckbox(root, "Debuffs", AURA_COL_X, DEBUFF_TOGGLE_Y, "showFocusDebuffs", true, function()
        RefreshBlizzardAnchorState()
        if MMF_UpdateFocusAuras then
            MMF_UpdateFocusAuras()
        end
    ...
    local blizzardAnchorCheckbox = CreateMinimalCheckbox(root, "Blizzard Anchor", AURA_COL_X + 136, BUFF_TOGGLE_Y, "focusUseBlizzardAuraAnchoring", false, function()
    ...
    RefreshBlizzardAnchorState = function()
        ...
        if not allowAnchor and blizzardAnchorCheckbox and blizzardAnchorCheckbox.checkbox and blizzardAnchorCheckbox.checkbox:GetChecked() then
            ...
            MattMinimalFramesDB.focusUseBlizzardAuraAnchoring = false
    ...
    local auraTypeOptions = {
        { value = "buff", label = "Focus Buffs" },
        { value = "debuff", label = "Focus Debuffs" },
    }
    MattMinimalFramesDB.focusAuraOffsetType = MattMinimalFramesDB.focusAuraOffsetType or "buff"
    if MattMinimalFramesDB.focusAuraOffsetType ~= "buff" and MattMinimalFramesDB.focusAuraOffsetType ~= "debuff" then
        MattMinimalFramesDB.focusAuraOffsetType = "buff"
    end
    ...
    local auraTypeDropdown = MMF_CreateMinimalDropdown(root, popup, {
        ...
        settingKey = "focusAuraOffsetType",
        x = AURA_COL_X,
        y = AURA_TYPE_Y,
        ...
        getValue = function()
            return MattMinimalFramesDB.focusAuraOffsetType
        end,
        onSelect = function(value)
            MattMinimalFramesDB.focusAuraOffsetType = value
            SyncAuraOffsetSliders()
        end,
    })
    dropdownLists.focusAuraTypeList = auraTypeDropdown.list
    ...
    MattMinimalFramesDB.focusBuffAuraDirection = NormalizeAuraDirection(MattMinimalFramesDB.focusBuffAuraDirection, "left_down")
    MattMinimalFramesDB.focusDebuffAuraDirection = NormalizeAuraDirection(MattMinimalFramesDB.focusDebuffAuraDirection, "right_up")

    local function RefreshAuraDirection()
        if MMF_UpdateAuraLayout then
            MMF_UpdateAuraLayout()
        elseif MMF_UpdateFocusAuras then
            MMF_UpdateFocusAuras()
        end
    end
    ...
    local buffDirectionDropdown = MMF_CreateMinimalDropdown(root, popup, {
        accentColor = ACCENT_COLOR,
        settingKey = "focusBuffAuraDirection",
        x = AURA_COL_X,
        y = -164,
        width = AURA_COL_WIDTH,
        labelWidth = 90,
        ...
        getValue = function()
            return NormalizeAuraDirection(MattMinimalFramesDB.focusBuffAuraDirection, "left_down")
        end,
        onSelect = function(value)
            MattMinimalFramesDB.focusBuffAuraDirection = NormalizeAuraDirection(value, "left_down")
            RefreshAuraDirection()
            RefreshPositionResetButtons()
        end,
    })
    dropdownLists.focusBuffAuraDirectionList = buffDirectionDropdown.list

    local debuffDirectionDropdown = MMF_CreateMinimalDropdown(root, popup, {
        accentColor = ACCENT_COLOR,
        settingKey = "focusDebuffAuraDirection",
        x = AURA_COL_X,
        y = -192,
        width = AURA_COL_WIDTH,
        labelWidth = 90,
        ...
        options = auraDirectionOptions,
        getValue = function()
            return NormalizeAuraDirection(MattMinimalFramesDB.focusDebuffAuraDirection, "right_up")
        end,
        onSelect = function(value)
            MattMinimalFramesDB.focusDebuffAuraDirection = NormalizeAuraDirection(value, "right_up")
            RefreshAuraDirection()
            RefreshPositionResetButtons()
        end,
    })
    dropdownLists.focusDebuffAuraDirectionList = debuffDirectionDropdown.list
    ...
    local function GetAuraOffsetKeys()
        if MattMinimalFramesDB.focusAuraOffsetType == "debuff" then
            return "focusDebuffXOffset", "focusDebuffYOffset", 3, 27
        end
        return "focusBuffXOffset", "focusBuffYOffset", -2, -6
    end

    local auraXSlider = CreateMinimalSlider(root, "X Offset", AURA_COL_X, -108, AURA_COL_WIDTH, "__tempFocusAuraOffsetX", -200, 200, 1, -2, function(value)
        local xKey, yKey = GetAuraOffsetKeys()
        MattMinimalFramesDB[xKey] = value
        if xKey == "focusDebuffXOffset" then
            if MMF_UpdateFocusDebuffPosition then
                MMF_UpdateFocusDebuffPosition(value, MattMinimalFramesDB[yKey] or 27)
            end
        else
            if MMF_UpdateFocusBuffPosition then
                MMF_UpdateFocusBuffPosition(value, MattMinimalFramesDB[yKey] or -6)
            end
        end
        RefreshPositionResetButtons()
    end, true)

    local auraYSlider = CreateMinimalSlider(root, "Y Offset", AURA_COL_X, -132, AURA_COL_WIDTH, "__tempFocusAuraOffsetY", -200, 200, 1, -6, function(value)
        local xKey, yKey = GetAuraOffsetKeys()
        MattMinimalFramesDB[yKey] = value
        if yKey == "focusDebuffYOffset" then
            if MMF_UpdateFocusDebuffPosition then
                MMF_UpdateFocusDebuffPosition(MattMinimalFramesDB[xKey] or 3, value)
            end
        else
            if MMF_UpdateFocusBuffPosition then
                MMF_UpdateFocusBuffPosition(MattMinimalFramesDB[xKey] or -2, value)
            end
        end
        RefreshPositionResetButtons()
    end, true)
    ...
    local function ResetAuraPosition(kind)
        if kind == "debuff" then
            ...
            if MMF_UpdateFocusDebuffPosition then
                MMF_UpdateFocusDebuffPosition(x, y)
            end
            if debuffDirectionDropdown and debuffDirectionDropdown.SetSelectedValue then
                debuffDirectionDropdown.SetSelectedValue(MattMinimalFramesDB.focusDebuffAuraDirection)
            end
            if MattMinimalFramesDB.focusAuraOffsetType == "debuff" then
                SyncAuraOffsetSliders()
            end
            ...
            if MMF_UpdateFocusBuffPosition then
                MMF_UpdateFocusBuffPosition(x, y)
            end
            if buffDirectionDropdown and buffDirectionDropdown.SetSelectedValue then
                buffDirectionDropdown.SetSelectedValue(MattMinimalFramesDB.focusBuffAuraDirection)
            end
            if MattMinimalFramesDB.focusAuraOffsetType ~= "debuff" then
                SyncAuraOffsetSliders()
            end
    ...
    auraTitle:SetText("FOCUS AURA APPEARANCE")

    local appearanceTypeOptions = {
        { value = "buff", label = "Focus Buffs" },
        { value = "debuff", label = "Focus Debuffs" },
    }
    MattMinimalFramesDB.focusAuraAppearanceType = MattMinimalFramesDB.focusAuraAppearanceType or "buff"
    if MattMinimalFramesDB.focusAuraAppearanceType ~= "buff" and MattMinimalFramesDB.focusAuraAppearanceType ~= "debuff" then
        MattMinimalFramesDB.focusAuraAppearanceType = "buff"
    end

    local SyncAuraAppearanceSliders = function() end
    local appearanceTypeDropdown = MMF_CreateMinimalDropdown(root, popup, {
        accentColor = ACCENT_COLOR,
        settingKey = "focusAuraAppearanceType",
        x = AURA_COL_X,
        y = -322,
        width = AURA_COL_WIDTH,
        labelWidth = 112,
        ...
        getValue = function()
            return MattMinimalFramesDB.focusAuraAppearanceType
        end,
        onSelect = function(value)
            MattMinimalFramesDB.focusAuraAppearanceType = value
            SyncAuraAppearanceSliders()
        end,
    })
    dropdownLists.focusAuraAppearanceTypeList = appearanceTypeDropdown.list

    local function GetAuraAppearanceKeys()
        if MattMinimalFramesDB.focusAuraAppearanceType == "debuff" then
            return "focusDebuffAuraIconSize", "focusDebuffAuraIconsPerRow", "focusDebuffAuraRows", 18, 4, 3
        end
        return "focusBuffAuraIconSize", "focusBuffAuraIconsPerRow", "focusBuffAuraRows", 18, 4, 3
    end
    ...
    auraIconSizeSlider = CreateMinimalSlider(root, "Icon Size", AURA_COL_X, -346, AURA_COL_WIDTH, "__tempFocusAuraAppearanceIconSize", 12, 40, 1, 18, function(value)
    ...
    auraIconsPerRowSlider = CreateMinimalSlider(root, "Icons Per Row", AURA_COL_X, -370, AURA_COL_WIDTH, "__tempFocusAuraAppearancePerRow", 1, 16, 1, 4, function(value)
    ...
    auraRowsSlider = CreateMinimalSlider(root, "Rows", AURA_COL_X, -394, AURA_COL_WIDTH, "__tempFocusAuraAppearanceRows", 1, 16, 1, 3, function(value)
    ...
    MattMinimalFramesDB.__tempFocusAuraOffsetX = nil
    MattMinimalFramesDB.__tempFocusAuraOffsetY = nil
    MattMinimalFramesDB.__tempFocusAuraAppearanceIconSize = nil
    MattMinimalFramesDB.__tempFocusAuraAppearancePerRow = nil
    MattMinimalFramesDB.__tempFocusAuraAppearanceRows = nil
end
